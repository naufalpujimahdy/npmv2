...
const contentTypes = [
  'PAGE', 'SECTION', 'PROJECT', 'EXPERIENCE', 'POST', 'SOCIAL', 'LINK',
...
type ValidateContentOptions = { partial?: boolean };
...
export function validateContentInput(
  ...
  if (!partial) {
    if (!isNonEmptyString(input.slug)) throw new ApiError(400, 'slug wajib diisi.');
    if (!isNonEmptyString(input.title)) throw new ApiError(400, 'title wajib diisi.');
    if (input.type === undefined) throw new ApiError(400, 'type wajib diisi.');
  }

  const data: Record<string, unknown> = {};

  if (input.type !== undefined) data.type = parseEnumValue(input.type, contentTypes, 'type');
  if (input.status !== undefined) data.status = parseEnumValue(input.status, contentStatuses, 'status');

  if (input.slug !== undefined) {
    if (!isNonEmptyString(input.slug)) throw new ApiError(400, 'slug wajib berupa string non-kosong.');
    const normalizedSlug = String(input.slug).trim().toLowerCase();
    if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(normalizedSlug)) {
      throw new ApiError(400, 'slug hanya boleh berisi huruf kecil, angka, dan tanda hubung.');
    }
    data.slug = normalizedSlug;
  }

  if (input.title !== undefined) {
    if (!isNonEmptyString(input.title)) throw new ApiError(400, 'title wajib berupa string non-kosong.');
    data.title = String(input.title).trim();
  }

  if (input.summary !== undefined) data.summary = normalizeOptionalString(input.summary);
  if (input.body !== undefined) data.body = normalizeOptionalString(input.body);

  if (input.locale !== undefined) {
    if (!isNonEmptyString(input.locale)) throw new ApiError(400, 'locale wajib berupa string non-kosong.');
    data.locale = String(input.locale).trim();
  }

  if (input.section !== undefined) data.section = normalizeOptionalString(input.section);

  if (input.coverImageUrl !== undefined) {
    const coverImageUrl = normalizeOptionalString(input.coverImageUrl);
    if (coverImageUrl && !URL.canParse(coverImageUrl) && !coverImageUrl.startsWith('/')) {
      throw new ApiError(400, 'coverImageUrl harus berupa URL absolut atau path relatif.');
    }
